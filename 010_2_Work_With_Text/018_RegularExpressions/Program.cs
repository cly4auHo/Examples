...
/*
          ...
          подстроку-вставку в третем параметре (Test).
          ...
          Этот пример вернёт строку test456firststr123secondstr он изменит шаблон так,
          что подстрока 123 и 456 поменяются местами. 
  
          В этом же примере используется новая конструкция {1,2} и подобные,
          она определяет кол-во повторов нужного нам элемента(\S, \d, etc). Формат:
          {min,max}.
          Пример: \d{2,4} - числа длиной от двух до четырёх символов
          (12, 123, 1234) 12345 - уже не выходит.
*/
namespace _018_RegularExpressions
{
    class Program
    {
        static void Main()
        {
            ...
            // Переменная внутри запроса ?<месяц>, тип - d{1,2} - цифровой, 
            // одна или две цифры
            Console.WriteLine(
                Regex.Replace("7/04/1993",                               // Исходная строка.
                 @"(?<месяц>\d{1,2})/(?<день>\d{1,2})\/(?<год>\d{2,4})",  // Шаблон.
                 "${день}-${месяц}-${год}"));                             // Новый формат.

            // ?<var1>\d+ - шаблонная переменная
            Console.WriteLine(
                Regex.Replace(@"test123firststr456secondstr",       // Исходная строка.
                @"test(?<var1>\d+)firststr(?<var2>\d+)secondstr",   // Шаблон.
                @"test${var2}firststr${var1}secondstr"));           // Новый формат.

            Console.WriteLine(
                Regex.Replace("5 is less than 10",            // Исходная строка.
                @"\d",                                        // Шаблон.
